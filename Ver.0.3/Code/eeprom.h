...
#define SWBS                                                                                       \
  0x40 /*软件复位选择控制位 0：软件复位后从用户代码开始执行程序          \
        1：软件复位后从系统 ISP 监控代码区开始执行程序 Software boot selection \
        control bit, which should be used with SWRST 0 : Execute the program                       \
        from the user code area after the software reset 1 : Execute the                           \
        program from the ISP memory area after the software reset*/
#define SWRST                                                       \
  0x20 /*软件复位控制位 0：无动作 1：产生软件复位 \
       Software reset trigger control                               \
       0 : No operation                                             \
       1: Generate software reset.*/
#define CMD_FAIL                                                                               \
  0x10 /*EEPROM操作失败状态位，需要软件清零 0： EEPROM 操作正确 1： EEPROM \
       操作失败 Command fail status bit for EEPROM operation which should be               \
       cleared by software 0 : EEPROM operation is right 0: EEPROM operation                   \
       is right*/
/*EEPROM操作的等待时间
